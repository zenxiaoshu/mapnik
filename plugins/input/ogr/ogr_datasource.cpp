...
void ogr_datasource::bind() const
{
    ...
    else
    {
        if (log_enabled_)
        {
            std::clog << "Mapnik LOG> ogr_datasource: no ogrindex file found for " << dataset_name_
...
#ifdef MAPNIK_LOG
                if (log_enabled_) std::clog << "Mapnik LOG> ogr_datasource: Unhandled type_oid=" << type_oid << std::endl;
#endif
                break;
